...
- Editor domain reload: `KeepNativeSessionsAcrossReload` detaches native `OrtSession` / `OrtEnv` handles before unload and wraps them after, so a script compile does not rebuild multi-GB graphs. Default `OrtEnv` (no custom logger fn ptr). `UnloadModels()` still releases.
