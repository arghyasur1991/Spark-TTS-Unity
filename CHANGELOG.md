...
- Editor domain reload: `KeepNativeSessionsAcrossReload` detaches native `OrtSession` / `OrtEnv` handles before unload and wraps them after, so a script compile does not rebuild multi-GB graphs. Default `OrtEnv` (no custom logger fn ptr — a Unity callback here SIGSEGVs after reload). Embedding npy + CP projection tables are still managed (~1.5 GB) and rebuild on the thread pool after wrap so `InitializeOnLoad` / Play does not block. `UnloadModels()` still releases.
