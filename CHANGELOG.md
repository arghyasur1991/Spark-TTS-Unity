...
- `LoadSession` / `AdoptSession` / tokenizer / `Synthesize` log wall times so a deferred first `InferenceSession` is visible (CustomVoice graphs stay closed until `GetSession`).
